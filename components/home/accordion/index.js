...
const Accordion = () => {
  ...
  return (
    <section className="my-28 flex flex-col gap-1 px-24 text-white">
      {accordions.map((section, index) => (
        <div
          key={index}
          onClick={() => toggleAccordion(index)}
          className={`cursor-pointer rounded-[3px] bg-[#201f1f] ${
            activeIndex.includes(index) ? "max-h-[1000px]" : "max-h-[52px]"
          } overflow-hidden transition-all duration-500`}
        >
          <div
            className={`${
              activeIndex.includes(index) ? " " : "border-none"
            } p-4`}
          >
            <button
              className={`duration-350 mb-2 flex w-full justify-between transition-all`}
            >
              <h2
                className={`aref tracking-[0.05em] ${
            ...
            <div className="poppins text-[14px] text-[#ccccccda]">
              {section.content}
            </div>
          </div>
        </div>
